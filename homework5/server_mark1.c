...
int main(int argc, char **argv)
{
  ...
  // {
  //   reader = fread(buf, 1, BUF_SIZE, fp);
  //   printf("%d : read %lld size from the file \n", ii, reader);
  // }
  fclose(fp);

  printf("fileSize: %lld %ld", fileSize, sizeof(fileSize));
  ...
  while(1)
  {
    ...
    {
      printf("receive success %d", new_fd);
      // send(new_fd, "hello you", 9, 0);
